...
def generate_research_gpt_summary():
    """Generate ResearchGPT summary - for scheduler use"""
    try:
        # Combine all research sources
        all_papers = PaperGPT.objects.all()
        ml_models = SnowAIMLModelLogEntry.objects.all()
        ...
        try:
            summary_obj, created = SnowAIResearchGPTSummary.objects.get_or_create(
                ...
                defaults={
                    ...
                    'average_model_sharpe': avg_sharpe,
                    'most_common_model_type': model_types[0]['snowai_model_type'] if model_types else 'N/A',
                    'primary_financial_market': financial_markets[0]['snowai_financial_market_type'] if financial_markets else 'N/A',
                    'future_directions': future_directions,
                }
            )
            ...
            if not created:
                summary_obj.summary_text = ai_summary
                summary_obj.total_papers = total_papers
                summary_obj.total_ml_models = total_ml_models
                summary_obj.total_backtests = total_backtests
                summary_obj.future_directions = future_directions
                summary_obj.updated_at = datetime.now()
                summary_obj.save()
        except:
            # If ResearchGPT model doesn't exist, just log success without saving
            pass

        return True

    except Exception as e:
        logger.error(f'Error in generate_research_gpt_summary: {e}')
        return False
